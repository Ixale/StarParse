...
public class ScumAndVillainy extends Raid {
	...
	private String getNewPhaseNameForStyrak(final Event e, final String currentPhaseName, final RaidBoss boss) {
		...
		// mass storm
		Helpers.isAbilityEqual(e, 3218004246528000L)
			// adds
			|| Helpers.isSourceEqual(e, 3142326922772480L)
			|| Helpers.isTargetEqual(e, 3142326922772480L)
			|| (phaseTimers.containsKey("CM") && phaseTimers.get("CM") < e.getTimestamp() - 35 * 1000))) { // should be dead in 35s anyway (prevents infinite phase for healers)
			phaseTimers.remove("CM");
			return "Boss";
		}

		if (!"CM".equals(currentPhaseName) && (Helpers.isSourceEqual(e, STYRAK_MANIFESTATION) || Helpers.isTargetEqual(e, STYRAK_MANIFESTATION))) {
		...
		if (!"Burn".equals(currentPhaseName) && (Helpers.isSourceEqual(e, 3225679353085952L) || Helpers.isTargetEqual(e, 3225679353085952L))) {
			return "Burn";
		}
	...
	public static class StyrakManifestationChallenge extends CombatChallenge {
		public StyrakManifestationChallenge() {
			super(RaidChallengeName.StyrakManifestation,
				...
				Arrays.asList(new Object[]{STYRAK_MANIFESTATION}),
				"target_guid IN (?)");
		}
	}
}
