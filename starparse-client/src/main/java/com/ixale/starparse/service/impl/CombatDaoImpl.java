...
public class CombatDaoImpl extends H2Dao implements CombatDao {
	...
	private static final String 
  		SQL_INSERT = "INSERT INTO combats"
				...
				+ ", threat, threat_positive, tps, discipline, is_pvp"
				+ ", is_running"
				+ ", combat_id)"
				+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
		...
		SQL_GET_STATS_TICKS = "WITH RECURSIVE subs (event_id, timestamp, next_id, duration"
				...
				+ "SELECT event_id, timestamp, (event_id + 1) AS next_id"
					+ ", 0 AS duration"
					+ ", (CASE WHEN source_type = 1 AND target_type != 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage"
					+ ", (CASE WHEN source_type = 1 AND target_type != 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage_sub"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal_sub"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND effective_heal IS NOT NULL THEN effective_heal ELSE 0 END) AS effective_heal"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND effective_heal IS NOT NULL THEN effective_heal ELSE 0 END) AS effective_heal_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage_taken"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage_taken_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" AND absorbed IS NOT NULL THEN absorbed ELSE 0 END) AS absorbed"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" AND absorbed IS NOT NULL THEN absorbed ELSE 0 END) AS absorbed_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal_taken"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal_taken_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND effective_heal IS NOT NULL THEN effective_heal ELSE 0 END) AS effective_heal_taken"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND effective_heal IS NOT NULL THEN effective_heal ELSE 0 END) AS effective_heal_taken_sub"
					+ ", (CASE WHEN target_type = 1 AND source_type != 1 AND effect_guid = "+EntityGuid.Heal+" AND effective_heal IS NULL THEN 0 ELSE (0"
						+ "+ (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END)"
						+ "+ (CASE WHEN target_type = 1 AND action_guid = "+EntityGuid.Spend+" AND effect_guid = "+EntityGuid.HealthPoint+" THEN value ELSE 0 END)"
						+ "- (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND effective_heal IS NOT NULL THEN effective_heal ELSE 0 END)"
						+ "- (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" AND absorbed IS NOT NULL THEN absorbed ELSE 0 END)"
						+ "- (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND ability_guid IN ("+EntityGuid.Enure+", "+EntityGuid.EnduringPain+") THEN value ELSE 0 END)"
				+ ") END) as health_sub"
				+ " FROM events s "
				+ " WHERE %event_from"
			+ " UNION ALL"
				+ " SELECT e.event_id, e.timestamp, (e.event_id + 1) AS next_id"
					...
					+ ", (CASE WHEN source_type = 1 AND target_type != 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage"
					+ ", (CASE WHEN source_type = 1 AND target_type != 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) + s.damage_sub AS damage_sub"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) + s.heal_sub AS heal_sub"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND e.effective_heal  IS NOT NULL THEN e.effective_heal ELSE 0 END) AS effective_heal"
					+ ", (CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND e.effective_heal  IS NOT NULL THEN e.effective_heal ELSE 0 END) + s.effective_heal_sub AS effective_heal_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage_taken"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) + s.damage_taken_sub AS damage_taken_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" AND e.absorbed IS NOT NULL THEN e.absorbed ELSE 0 END) AS absorbed"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" AND e.absorbed IS NOT NULL THEN e.absorbed ELSE 0 END) + s.absorbed_sub AS absorbed_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal_taken"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) + s.heal_taken_sub AS heal_taken_sub"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND e.effective_heal IS NOT NULL THEN e.effective_heal ELSE 0 END) AS effective_heal_taken"
					+ ", (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND e.effective_heal IS NOT NULL THEN e.effective_heal ELSE 0 END) + s.effective_heal_taken_sub AS effective_heal_taken_sub"
					+ ", (CASE WHEN target_type = 1 AND source_type != 1 AND effect_guid = "+EntityGuid.Heal+" "
						...
						+ "+ (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END)"
						+ "+ (CASE WHEN target_type = 1 AND action_guid = "+EntityGuid.Spend+" AND effect_guid = "+EntityGuid.HealthPoint+" THEN value ELSE 0 END)"
						+ "- (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND e.effective_heal IS NOT NULL THEN e.effective_heal ELSE 0 END)"
						+ "- (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" AND e.absorbed IS NOT NULL THEN e.absorbed ELSE 0 END)"
						+ "- (CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" AND ability_guid IN ("+EntityGuid.Enure+", "+EntityGuid.EnduringPain+") THEN e.value ELSE 0 END)"
					+ ") END) as health_sub"
				+ " FROM subs s"
		...
		SQL_GET_STATS_SUMS_CACHED = "SELECT c.*, DATEDIFF('MILLISECOND', time_from, time_to) duration"
				+ " FROM combats c WHERE combat_id = ?",
		...
		SQL_GET_EVENTS = "SELECT e.*"
				...
				+ " FROM events e"
				+ " LEFT JOIN absorptions a ON (e.target_type = 1 AND e.absorbed IS NOT NULL AND a.event_id = e.event_id)"
		...
		SQL_GET_STATS_SUMS = "SELECT duration, time_to"   
				...
				+", threat, threat_positive"
			+" FROM ("
				+"SELECT LEAST(36000000, GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE MAX(timestamp) END))) AS duration"
					+", MAX(timestamp) time_to"
					+", SUM(CASE WHEN effect_guid = "+EntityGuid.AbilityActivate+" THEN 1 ELSE 0 END) AS actions"
					+", SUM(CASE WHEN source_type = 1 AND target_type != 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage"
					+", SUM(CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal"
					+", SUM(CASE WHEN source_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN effective_heal ELSE 0 END) AS effective_heal"
		
					+", SUM(CASE WHEN source_type = 1 THEN effective_threat ELSE 0 END) AS threat"
					+", SUM(CASE WHEN source_type = 1 AND threat > 0 THEN threat ELSE 0 END) AS threat_positive"
			  	
					+", SUM(CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" THEN value ELSE 0 END) AS damage_taken"
					+", SUM(CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Damage+" AND absorbed IS NOT NULL THEN absorbed ELSE 0 END) AS absorbed"
					+", SUM(CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN value ELSE 0 END) AS heal_taken"
					+", SUM(CASE WHEN target_type = 1 AND effect_guid = "+EntityGuid.Heal+" THEN effective_heal ELSE 0 END) AS effective_heal_taken"
					+" FROM EVENTS e"
				+" WHERE (e.event_id BETWEEN ? AND ?)"
				+")",
		...
		SQL_GET_MITIGATION_STATS_SUMS = "SELECT duration, ticks, damage"
			...
			+" FROM ("
				+"SELECT GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE MAX(e.timestamp) END)) AS duration"
				 ...
				 +" LEFT JOIN events ae ON (ae.event_id = af.event_id_from)"
				 +" WHERE (e.event_id BETWEEN ? AND ?)"
				 + " AND e.target_type = 1 AND e.effect_guid = "+EntityGuid.Damage+""
				+")",
		...
		SQL_GET_DOT_NAMES = "SELECT e.effect_guid, e.ability_name, a.actions, a.ability_guid FROM ("
				...
				+" WHERE f.event_id_to > ? AND f.event_id_from < ?" 
				+" AND source_type = 1"
			...
			+" INNER JOIN ("
				...
				+" FROM events"
				+" WHERE (event_id BETWEEN ? AND ? AND timestamp >= ?)"
				+" AND source_type = 1 AND effect_guid = "+EntityGuid.AbilityActivate   
		...
		SQL_GET_DAMATE_DEALT_SUMS = 
			...
			+" FROM events e"
			+" WHERE (event_id BETWEEN ? AND ? AND timestamp >= ?)" // catch casts before combat start
				+" AND source_type = 1" 
				+" AND ((effect_guid = "+EntityGuid.Damage+" AND target_type != 1) OR (effect_guid = "+EntityGuid.AbilityActivate+"))"
				+" AND ability_guid IN (SELECT ability_guid FROM events WHERE effect_guid = "+EntityGuid.Damage+" AND source_type = 1 AND event_id BETWEEN ? AND ?)",
	  	...
	  	SQL_GET_DAMAGE_DEALT_TOTALS = "SELECT a.*"
				...
				+", ROUND(a.total * 1000.0 / GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE t.timestamp END))) dps"
			...
			+" SELECT %pivots_cols, " // ability_name, ability_guid
				+ SQL_GET_DAMATE_DEALT_SUMS
			+" %pivots_where" // target_type != 1
			+" GROUP BY %pivots_group" // ability_name, ability_guid
			+" ) a"
			+" INNER JOIN ("
  				+" SELECT SUM(CASE WHEN effect_guid = "+EntityGuid.Damage+" AND source_type = 1 AND target_type != 1 THEN value ELSE 0 END) total, MAX(timestamp) timestamp"
  				+" FROM events e"
  				+" WHERE e.event_id BETWEEN ? AND ?"
			+") t"
		...
		SQL_GET_DAMAGE_DEALT_SIMPLE = "SELECT a.*"
			...
			+" SELECT " // ability_name, ability_guid
				+ SQL_GET_DAMATE_DEALT_SUMS
				+" AND e.target_type != 1" // target_type != 1
			+" ) a"
			+" INNER JOIN ("
		...
		SQL_GET_HEALING_DONE_SUMS = 
				", SUM(CASE WHEN effect_guid = "+EntityGuid.AbilityActivate+" THEN 1 ELSE 0 END) actions"
				+", SUM(CASE WHEN effect_guid = "+EntityGuid.Heal+" THEN 1 ELSE 0 END) ticks"
				+", SUM(CASE WHEN NOT is_crit THEN 1 ELSE 0 END) ticks_normal"
				+", SUM(CASE WHEN is_crit THEN 1 ELSE 0 END) ticks_crit"

				+", SUM(CASE WHEN NOT is_crit THEN value ELSE 0 END) total_normal"
				+", SUM(CASE WHEN is_crit THEN value ELSE 0 END) total_crit"
				+", SUM(CASE WHEN effective_heal IS NOT NULL THEN effective_heal ELSE 0 END) total_effective"
				+", MAX(value) max, SUM(value) total"
				+", MIN(timestamp) sub_time_from, MAX(timestamp) sub_time_to"
			+" FROM events e"
			+" WHERE event_id BETWEEN ? AND ?"
				...
				+" AND ability_guid IN (SELECT ability_guid FROM events WHERE effect_guid = "+EntityGuid.Heal+" AND source_type = 1 AND event_id BETWEEN ? AND ?)",

		SQL_GET_HEALING_DONE_TOTALS = "SELECT a.*"
				+", (CASE WHEN ticks > 0 THEN ROUND(a.total * 1.0 / ticks) ELSE 0 END) avg_normal"
				+", (CASE WHEN ticks_crit > 0 THEN ROUND(total_crit * 1.0 / ticks_crit) ELSE 0 END) avg_crit"
				+", (CASE WHEN ticks > 0 THEN ROUND(ticks_crit * 100.0 / ticks, 2) ELSE 0 END) AS pct_crit"
				+", (CASE WHEN a.total > 0 THEN ROUND(a.total_effective * 100.0 / a.total, 1) ELSE 0 END) pct_effective"
				+", (CASE WHEN t.total_effective > 0 THEN ROUND(a.total_effective * 100.0 / t.total_effective, 1) ELSE 0 END) pct_total"
				+", ROUND(a.total * 1000.0 / GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE t.timestamp END))) hps"
				+", ROUND(a.total_effective * 1000.0 / GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE t.timestamp END))) ehps"
			...
			+" ) a"
			+" INNER JOIN ("
	  			+" SELECT SUM(CASE WHEN effect_guid = "+EntityGuid.Heal+" AND source_type = 1 THEN value ELSE 0 END) total"
	  				+", SUM(CASE WHEN effective_heal IS NOT NULL AND source_type = 1 THEN effective_heal ELSE 0 END) total_effective"
		...
		SQL_GET_DAMAGE_TAKEN_SUMS = 
	  	...
	  	SQL_GET_DAMAGE_TAKEN_TOTALS = "SELECT a.*"
				...
				+", (CASE WHEN ticks > 0 THEN ROUND(ticks_miss * 100.0 / ticks, 2) ELSE 0 END) AS pct_miss"
				+", (CASE WHEN t.total > 0 THEN ROUND(a.total * 100.0 / t.total, 1) ELSE 0 END) AS pct_total"
				+", ROUND(a.total * 1000.0 / GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE t.timestamp END))) dtps"
			...
			// total
			+" INNER JOIN ("
  				+" SELECT SUM(CASE WHEN effect_guid = "+EntityGuid.Damage+" AND target_type = 1 THEN value ELSE 0 END) total, MAX(timestamp) timestamp"
  				+" FROM events e"
  				+" WHERE e.event_id BETWEEN ? AND ?"
			+") t"
			// sources
		...
		SQL_GET_HEALING_TAKEN_SUMS = 
			...
			+ " FROM ("
				...
				+" WHERE ea.event_id BETWEEN ? AND ?"
					+" AND e.target_type = 1) e",
	  	...
	  	SQL_GET_HEALING_TAKEN_TOTALS = "SELECT a.*"
				...
				+", ROUND(a.total * 1000.0 / GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE t.timestamp END))) htps"
				+", ROUND(a.total_effective * 1000.0 / GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE t.timestamp END))) ehtps"
				+", ROUND(a.absorbed * 1000.0 / GREATEST(1000, DATEDIFF('MILLISECOND', ?, CASE WHEN ? IS NOT NULL THEN ? ELSE t.timestamp END))) aps"
				+", ts.source_time_from, ts.source_time_to, a.sub_time_from, a.sub_time_to"
			// sums
			+" FROM ("
			+" SELECT %pivots_cols" // ability_name, ability_guid
			...
			// total
			+" INNER JOIN ("
	  			+" SELECT SUM(CASE WHEN effect_guid = "+EntityGuid.Heal+" AND target_type = 1 THEN value ELSE 0 END) total"
	  				+", SUM(CASE WHEN effective_heal IS NOT NULL AND target_type = 1 THEN effective_heal ELSE 0 END) total_effective"
	  				+", SUM(CASE WHEN absorbed IS NOT NULL AND target_type = 1 THEN absorbed ELSE 0 END) total_absorbed"
	  				+", MAX(timestamp) timestamp"
	  			+" FROM events e"
	  			+" WHERE e.event_id BETWEEN ? AND ?"
			+") t"
		...
		SQL_GET_COMBAT_EFFECTS =
				...
				+ " WHERE (f.event_id_to >= ? OR (f.event_id_to IS NULL AND f.event_id_from >= ?)) AND f.event_id_from <= ?"
		...
		SQL_GET_ABSORPTION_TAKEN_TOTALS = 
				...
				+ " WHERE ea.target_type = 1 "
					+ " AND ea.event_id BETWEEN ? AND ?"
				+ " GROUP BY e.source_name, e.ability_guid, e.ability_name",

		SQL_GET_COMBAT_CHALLENGES =
				"SELECT phase_id, name, type, combat_id, event_id_from, event_id_to, tick_from, tick_to"
				+ " FROM phases"
				+ " WHERE combat_id = ?"
					+ " AND (tick_to IS NULL OR tick_to >= ?) AND tick_from <= ?"
					+ " AND name IN (?)"
	...
	private final HashMap<String, CombatChallenge> phasesToChallenges = new HashMap<String, CombatChallenge>();
	private final HashMap<Integer, HashMap<Long, ChallengeStats>> cachedChallenges = new HashMap<Integer, HashMap<Long,ChallengeStats>>();
	...
	public void storeCombats(final List<Combat> combats, final Combat currentCombat) throws Exception {
		...
		ArrayList<Integer> storedCombatIds = new ArrayList<Integer>();
		Integer runningCombatId = null;
		runningCombatId = getJdbcTemplate().query("SELECT combat_id FROM combats WHERE is_running = TRUE", new ResultSetExtractor<Integer>() {
			@Override
			public Integer extractData(ResultSet rs) throws SQLException, DataAccessException {
				return rs.next() ? rs.getInt("combat_id") : null;
			}});
	...
	private void storeCombat(final Combat c, boolean create, boolean close) throws Exception {
		// resolve name
		final String combatName = getJdbcTemplate().query(SQL_GET_NAME, new Object[]{
				c.getEventIdFrom(),
				c.getEventIdTo() != null ? c.getEventIdTo() : Integer.MAX_VALUE 
			}, new ResultSetExtractor<String>() {
				@Override
				public String extractData(ResultSet rs) throws SQLException, DataAccessException {
					return rs.next() ? rs.getString("combat_name") : null;
				}});

		// gather statistics
		final CombatStats stats = getCombatStats(c, new CombatSelection(c.getEventIdFrom(), c.getEventIdTo(), null, null));
		...
		getJdbcTemplate().update(create ? SQL_INSERT : SQL_UPDATE, new Object[] {
				...
				c.getEventIdFrom(),
				c.getEventIdTo(),
				new Timestamp(c.getTimeFrom()),
				new Timestamp(c.getTimeTo() != null ? c.getTimeTo() : c.getTimeFrom() + stats.getTick()),
	...
	private Combat getCombat(Integer combatId) throws Exception {
		...
		try {
			return getJdbcTemplate().queryForObject(sql, new RowMapper<Combat>() {
				@Override
				public Combat mapRow(ResultSet rs, int rowNum) throws SQLException {

					final Combat c = new Combat(rs.getInt("combat_id"), rs.getInt("log_id"),
						getTimestamp(rs, "time_from").getTime(), rs.getInt("event_id_from"));
					...
					if (rs.getString("discipline") != null) {
						c.setDiscipline(CharacterDiscipline.valueOf(rs.getString("discipline")));
					}
					c.setIsPvp(getValueOrNull(rs, rs.getBoolean("is_pvp")));
	...
	public List<Combat> getCombats() throws Exception {
		return getJdbcTemplate().query(
				"SELECT combat_id, log_id, time_from, time_to, event_id_from, event_id_to"
				+ ", combat_name, raid_name, boss_name, discipline, is_pvp, is_running"
				+ " FROM combats"
				+ " ORDER BY combat_id ASC", 
				new RowMapper<Combat>() {
					@Override
					public Combat mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						final Combat c = new Combat(rs.getInt("combat_id"), rs.getInt("log_id"), 
							getTimestamp(rs, "time_from").getTime(), rs.getInt("event_id_from"));
						c.setName(rs.getString("combat_name"));
	...
	public CombatStats getCombatStats(final Combat combat, final CombatSelection combatSel) throws Exception {
		return getCombatStats(combat, combatSel, false);
	}

	private CombatStats getCombatStats(final Combat combat, final CombatSelection combatSel, final boolean realDuration) throws Exception {
		...
		try {
			return getJdbcTemplate().query(sql, args.toArray(), new ResultSetExtractor<CombatStats>() {
				@Override
				public CombatStats extractData(ResultSet rs) throws SQLException, DataAccessException {
					if (!rs.next()) {
						return null;
					}
					int duration;
					if (realDuration) {
						if (getTimestamp(rs, "time_to") != null) {
			...
			throw new Exception("Unable to get combat summary (" + Arrays.asList(args.toArray()) + "): " + e.getMessage(), e);
		}
	}

	@Override
	public List<Event> getCombatEvents(final Combat combat, final Set<Event.Type> filterFlags,
			final Actor filterSource, final Actor filterTarget, final String filterSearch, final CombatSelection combatSel) throws Exception {
		...
		for (final Event.Type t: filterFlags) {
			switch(t) {
			case DAMAGE_DEALT:
				pivotsWhere.append(" OR (e.source_type = 1 AND e.effect_guid = "+EntityGuid.Damage+")");
				break;
			case DAMAGE_TAKEN:
				pivotsWhere.append(" OR (e.target_type = 1 AND e.effect_guid = "+EntityGuid.Damage+")");
				break;
			case HEALING_DONE:
				pivotsWhere.append(" OR (e.source_type = 1 AND e.effect_guid = "+EntityGuid.Heal+")");
				break;
			case HEALING_TAKEN:
				pivotsWhere.append(" OR (e.target_type = 1 AND e.effect_guid = "+EntityGuid.Heal+")");
				break;
			case ACTIONS:
				pivotsWhere.append(" OR (e.source_type = 1 AND e.effect_guid = "+EntityGuid.AbilityActivate+")");
				break;
			case EVENT_SELF:
				pivotsWhere.append(" OR (e.target_type = 1"
			...
			case EVENT_OTHERS:
				pivotsWhere.append(" OR (e.target_type != 1"
						...
						+ " AND e.action_guid IN ("+EntityGuid.ApplyEffect+", "+EntityGuid.RemoveEffect+"))");
			case SIMPLIFIED:
				sqlBuilder.append(" AND e.action_guid NOT IN ("+EntityGuid.Spend+", "+EntityGuid.Restore+")");
				break;
			case COMBAT_EXIT:
			case DEATH:
			case NIM_CRYSTAL:
		...
		if (pivotsWhere.length() > 0) {
			...
			sqlBuilder.append(pivotsWhere.toString());
			sqlBuilder.append(")");
		}

		if (filterSource != null) {
			if (filterSource.getInstanceId() != null) {
				sqlBuilder.append(" AND e.source_instance = ?");
				args.add(filterSource.getInstanceId());
			} else if (filterSource.getGuid() != null) {
				sqlBuilder.append(" AND e.source_guid = ?");
				args.add(filterSource.getGuid());
			} else {
				sqlBuilder.append(" AND e.source_name = ?");
				args.add(filterSource.getName());
			}
		}

		if (filterTarget != null) {
			if (filterTarget.getInstanceId() != null) {
				sqlBuilder.append(" AND e.target_instance = ?");
				args.add(filterTarget.getInstanceId());
			} else if (filterTarget.getGuid() != null) {
				sqlBuilder.append(" AND e.target_guid = ?");
				args.add(filterTarget.getGuid());
			} else {
				sqlBuilder.append(" AND e.target_name = ?");
				args.add(filterTarget.getName());
			}
		}
		...
		if (filterSearch != null) {
			...
			for (final String f: filterSearch.split(" OR [ ]*")) {
				...
				sb.append("LOWER(e.source_name) LIKE ? OR e.source_guid LIKE ? OR e.source_instance LIKE ?"
					+ " OR LOWER(e.target_name) LIKE ? OR e.target_guid LIKE ? OR e.target_instance LIKE ?"
					+ " OR LOWER(e.ability_name) LIKE ? OR e.ability_guid LIKE ?"
					+ " OR LOWER(e.effect_name) LIKE ? OR e.effect_guid LIKE ?"
					+ " OR LOWER(e.action_name) LIKE ? OR e.action_guid LIKE ?"
					+ " OR LOWER(e.damage_name) LIKE ? OR e.damage_guid LIKE ?"
					+ " OR LOWER(e.mitigation_name) LIKE ? OR e.mitigation_guid LIKE ?"
					+ " OR LOWER(e.absorption_name) LIKE ? OR e.absorption_guid LIKE ?"
					+ " OR LOWER(e.reflect_name) LIKE ? OR e.reflect_guid LIKE ?");
				final String fs = "%" + f.toLowerCase() + "%";
			...
			sqlBuilder.append("AND (").append(sb.toString()).append(")");
		}

		return getJdbcTemplate().query(sqlBuilder.toString(), args.toArray(),
				new RowMapper<Event>() {
					@Override
	...
	public List<CombatTickStats> getCombatTicks(final Combat combat, final CombatSelection combatSel) throws Exception {
		...
		return getJdbcTemplate().query(sql, (Object[]) null, new RowMapper<CombatTickStats>() {
			@Override
			public CombatTickStats mapRow(ResultSet rs, int rowNum) throws SQLException {
				return new CombatTickStats(
	...
	public List<DamageDealtStats> getDamageDealtStatsSimple(final Combat combat, final CombatSelection combatSel) throws Exception {
		...
		final String sql = SQL_GET_DAMAGE_DEALT_SIMPLE;
		...
		final Object[] args = new Object[] {
				// duration
				bounds.timeFrom, bounds.timeTo, bounds.timeTo,
				// abilities - breakdown
				bounds.eventIdFrom - LOOKBEHIND_EVENTS, bounds.eventIdTo, lookbehind,
				bounds.eventIdFrom, bounds.eventIdTo,
				// total
				bounds.eventIdFrom, bounds.eventIdTo,
		};

		return getJdbcTemplate().query(sql, args, new RowMapper<DamageDealtStats>() {
			@Override
			public DamageDealtStats mapRow(ResultSet rs, int rowNum) throws SQLException {
				return new DamageDealtStats(
					"Total",
					"Total",
					0L,
					rs.getInt("actions"),
	...
	public List<DamageDealtStats> getDamageDealtStats(final Combat combat, boolean byTargetType, boolean byTargetInstance, boolean byAbility, 
		...
		final String sql = SQL_GET_DAMAGE_DEALT_TOTALS;
		...
		if (byTargetInstance) {
			...
			pivotsWhere.append(" AND e.target_type != 1");
		...
		return getJdbcTemplate().query(sql
					.replace("%pivots_cols", pivotsCols.toString().substring(2))
					.replace("%pivots_where", pivotsWhere.length() > 0 ? pivotsWhere.toString() : "")
					.replace("%pivots_group", pivotsGroup.length() > 0 ? pivotsGroup.toString().substring(2) : "1")
				, args,
				new RowMapper<DamageDealtStats>() {
					@Override
					public DamageDealtStats mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new DamageDealtStats(
	...
	@Override
	public List<HealingDoneStats> getHealingDoneStats(final Combat combat, boolean byTarget, boolean byAbility, final CombatSelection combatSel) throws Exception {

		final String sql = SQL_GET_HEALING_DONE_TOTALS;
		...
		return getJdbcTemplate().query(sql
					...
					.replace("%pivots_group", pivotsGroup.length() > 0 ? pivotsGroup.toString().substring(2) : "1")
				, args,
				new RowMapper<HealingDoneStats>() {
					@Override
					public HealingDoneStats mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new HealingDoneStats(
							rs.getString("target_name"),
							(rs.getString("dot_name") != null ? rs.getString("dot_name")+": " : "") + rs.getString("ability_name"),
	...
	@Override
	public CombatMitigationStats getCombatMitigationStats(final Combat combat, final CombatSelection combatSel) throws Exception {

		final String sql = SQL_GET_MITIGATION_STATS_SUMS;
		...
		return getJdbcTemplate().query(sql, args, new ResultSetExtractor<CombatMitigationStats>() {
				@Override
				public CombatMitigationStats extractData(ResultSet rs) throws SQLException, DataAccessException {
	...
	public List<Actor> getCombatActors(final Combat combat, final Actor.Role role, final CombatSelection combatSel) throws Exception {
		...
		return getJdbcTemplate().query(sql, args, new RowMapper<Actor>() {
					@Override
	...
	public List<DamageTakenStats> getDamageTakenStats(final Combat combat, boolean bySourceType, boolean bySourceInstance, boolean byAbility,
		...
		final String sql = SQL_GET_DAMAGE_TAKEN_TOTALS;
		...
		return getJdbcTemplate().query(sql
					...
					.replace("%pivots_group", pivotsGroup.length() > 0 ? pivotsGroup.toString().substring(2) : "1")
				, args,
				new RowMapper<DamageTakenStats>() {
					@Override
					public DamageTakenStats mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new DamageTakenStats(
	...
	public List<HealingTakenStats> getHealingTakenStats(final Combat combat, boolean bySource, boolean byAbility, final CombatSelection combatSel) throws Exception {
		
		final String sql = SQL_GET_HEALING_TAKEN_TOTALS;
		...
		return getJdbcTemplate().query(sql
					...
					.replace("%pivots_group", pivotsGroup.length() > 0 ? pivotsGroup.toString().substring(2) : "1")
				, args,
				new RowMapper<HealingTakenStats>() {
					@Override
					public HealingTakenStats mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new HealingTakenStats(
							rs.getString("source_name"),
							rs.getString("ability_name"),
							rs.getLong("ability_guid"),
							rs.getInt("ticks"),
	...
	@Override
	public List<AbsorptionStats> getAbsorptionStats(final Combat combat, final CombatSelection combatSel) throws Exception {
		...
		return getJdbcTemplate().query(SQL_GET_ABSORPTION_TAKEN_TOTALS, 
				args,
				new RowMapper<AbsorptionStats>() {
					@Override
					public AbsorptionStats mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new AbsorptionStats(
								rs.getString("source_name"),
								EntityGuid.fromGuid(rs.getLong("effect_guid")),
								rs.getInt("total")
							);
					}
				});
	...
	public List<ChallengeStats> getCombatChallengeStats(final Combat combat, final CombatSelection combatSel) throws Exception {
		...
		final ArrayList<Object> args = new ArrayList<Object>();
		args.add(combat.getCombatId());
		args.add(combatSel != null && combatSel.getTickFrom() != null ? combatSel.getTickFrom() : 0);
		args.add(combatSel != null && combatSel.getTickTo() != null ? combatSel.getTickTo() : Integer.MAX_VALUE);
		for (String c: phasesToChallenges.keySet()) {
			args.add(c);
		...
		return getJdbcTemplate().query(phasesToChallenges.size() == 1 
					? SQL_GET_COMBAT_CHALLENGES 
					: SQL_GET_COMBAT_CHALLENGES.replace("(?)", "("+sqlBuilder.toString()+")"), 
				args.toArray(),
				new RowMapper<ChallengeStats>() {
					...
					@Override
					public ChallengeStats mapRow(ResultSet rs, int rowNum) throws SQLException {
						try {
							noCache = false;
							if (combatSel != null) {
							...
							if (!noCache && cachedChallenges.containsKey(combat.getCombatId())) {
								if (cachedChallenges.get(combat.getCombatId()).containsKey(rs.getLong("tick_from"))) {
									return cachedChallenges.get(combat.getCombatId()).get(rs.getLong("tick_from"));
									// NOTREACHED
								}
							}
							...
							final CombatStats stats = getCombatStats(combat, challengeCombatSel, true);
							...
							if (!noCache && (getValueOrNull(rs, rs.getLong("tick_to")) != null)) {
								if (!cachedChallenges.containsKey(combat.getCombatId())) {
									cachedChallenges.put(combat.getCombatId(), new HashMap<Long,ChallengeStats>());									
								}
								cachedChallenges.get(combat.getCombatId()).put(rs.getLong("tick_from"), challengeStats);
							}
	...
	public List<Effect> getCombatEffects(final Combat combat, final CombatSelection combatSel) throws Exception {
		...
		Object[] args = new Object[]{
			bounds.eventIdFrom, bounds.eventIdFrom, bounds.eventIdTo,
			};

		return getJdbcTemplate().query(SQL_GET_COMBAT_EFFECTS, 
				args,
				new RowMapper<Effect>() {
					@Override
